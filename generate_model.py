...
Génère l'objet 3D animé du site AR, en deux formats à partir d'une seule description :
  ...
  assets/objet.glb   -> Android (WebXR / Scene Viewer) et aperçu 3D dans la page
  assets/objet.usdz  -> iPhone / iPad (AR Quick Look), animation incluse
...
KEYS = 24        # nombre de segments d'interpolation dans le glTF
...
SCENE = {"name": "Scene", "children": [
    ...
    {"name": "Floating", "bob": (0.175, 0.012), "children": [
...
def export_usda(meshes):
    ...
    def node_block(d, ind, is_root=False):
        ...
        if "rot" in d:
            ax, deg = d["rot"]
            L.append(f"{p}    double xformOp:rotate{ax.upper()} = {r6(deg)}"); order.append(f'"xformOp:rotate{ax.upper()}"')
        if "spin" in d:
            ax, revs = d["spin"]
            L.append(f"{p}    double xformOp:rotate{ax.upper()}.timeSamples = {{ 0: 0, {end}: {r6(360.0*revs)} }}")
            order.append(f'"xformOp:rotate{ax.upper()}"')
        if "bob" in d:
            y0, amp = d["bob"]
            step = end // KEYS
            samples = ", ".join(f"{i*step}: {vec((0.0, y0 + amp*math.sin(2*math.pi*i/KEYS), 0.0))}" for i in range(KEYS + 1))
...
if __name__ == "__main__":
    meshes = build_meshes()
    n = export_glb(meshes, os.path.join(OUT, "objet.glb"))
    usda = export_usda(meshes)
    if os.environ.get("KEEP_USDA"): open(os.path.join(OUT, "objet.usda"), "w").write(usda)  # débogage
    m = write_usdz([("objet.usda", usda.encode())], os.path.join(OUT, "objet.usdz"))
    print(f"objet.glb  : {n/1024:.0f} Ko")
    print(f"objet.usdz : {m/1024:.0f} Ko")
