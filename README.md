...
| `assets/objet.glb` | Modèle animé pour Android (WebXR, Scene Viewer) et l'aperçu 3D |
| `assets/objet.usdz` | Le même modèle animé pour l'iPhone (AR Quick Look) |
...
- Autre objet : remplacez `assets/objet.glb` (avec animation) et `assets/objet.usdz`, ou modifiez la
  scène dans `generate_model.py` puis relancez-le.
- Si l'animation ne se lit pas dans Quick Look sur iPhone, retirez l'attribut `ios-src` dans `index.html` :
  `<model-viewer>` fabrique alors lui-même un USDZ à partir du GLB (objet posé, mais sans animation).
